...
export default defineSchema({
  ...
  taskLists: defineTable({
    // Space-scoped lists are owned by a module (currently only the Baby log's
    // checklist cards). The Tasks module itself is still group-scoped, so
    // exactly one of spaceId / groupId is set.
    spaceId: v.optional(v.id('spaces')),
  ...
  babies: defineTable({
    spaceId: v.optional(v.id('spaces')),
    // Legacy scope, kept so pre-Space rows stay readable until they are
    // backfilled onto a spaceId. New rows always set spaceId.
    groupId: v.optional(v.id('groups')),
    name: v.string(),
