...
export const listByBaby = query({
  ...
  handler: async (ctx, args) => {
    await requireBabyAccess(ctx, args.spaceSlug, args.babyId)
...
async function requireEditableEvent(
  ...
  await requireBabyAccess(ctx, spaceSlug, event.babyId)
  return event
}
...
export const add = mutation({
  ...
  handler: async (ctx, args) => {
    const { user } = await requireBabyAccess(ctx, args.spaceSlug, args.babyId)
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    const event = await requireEditableEvent(ctx, args.spaceSlug, args.eventId)
...
export const remove = mutation({
  args: { spaceSlug: v.string(), eventId: v.id('babyEvents') },
  handler: async (ctx, args) => {
    await requireEditableEvent(ctx, args.spaceSlug, args.eventId)
