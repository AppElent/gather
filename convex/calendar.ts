...
export const listCalendars = query({
  ...
  returns: v.object({ page: v.array(calendarOutput), isDone: v.boolean(), continueCursor: v.string() }),
  handler: async (ctx, args) => {
    const { group } = await requireGroupBySlug(ctx, args.groupSlug)
    const page = await ctx.db.query('calendars').withIndex('by_group', (q) => q.eq('groupId', group._id)).order('asc').paginate(args.paginationOpts)
    return { page: page.page.map((calendar) => ({ ...calendar, color: calendar.color ?? 'home' as const })), isDone: page.isDone, continueCursor: page.continueCursor }
  },
})

export const preferences = query({
  args: { groupSlug: v.string() },
  returns: v.object({ view: calendarViewValidator, hiddenCalendarIds: v.array(v.id('calendars')), peopleFilter: v.union(v.null(), calendarPeopleFilterValidator) }),
  handler: async (ctx, args) => {
...
export const listEvents = query({
  ...
  returns: v.object({ page: v.array(eventOutput), isDone: v.boolean(), continueCursor: v.string() }),
  handler: async (ctx, args) => {
    const { calendar } = await calendarInGroup(ctx, args.groupSlug, args.calendarId)
    const page = await ctx.db.query('calendarEvents')
      .withIndex('by_calendar_date', (q) => q.eq('calendarId', calendar._id).gte('date', args.from).lt('date', args.toExclusive))
      .order('asc')
      .paginate({ ...args.paginationOpts, numItems: Math.min(args.paginationOpts.numItems, 100) })
    return { page: page.page.map((event) => eventForOutput(normalizedEvent(event, calendar))), isDone: page.isDone, continueCursor: page.continueCursor }
  },
})

export const updateCalendar = mutation({
  args: { groupSlug: v.string(), id: v.id('calendars'), color: calendarColorValidator },
  returns: v.null(),
  handler: async (ctx, args) => {
...
export const updateEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const destination = await calendarInGroup(ctx, args.groupSlug, args.calendarId)
    const currentRevision = current.event.revision ?? 0
    if (currentRevision !== args.expectedRevision) throw new Error('calendar:conflict')
    validateEventFields(args)
    await validateAssignees(ctx, destination.group._id, args.assigneeIds, current.event.assigneeIds ?? [])
    const patch = {
    ...
    const unchanged = current.event.calendarId === patch.calendarId && current.event.title === patch.title && current.event.date === patch.date && (current.event.allDay ?? (current.event.startMinutes === undefined && current.event.endMinutes === undefined)) === patch.allDay && current.event.startMinutes === patch.startMinutes && current.event.endMinutes === patch.endMinutes && JSON.stringify(current.event.assigneeIds ?? []) === JSON.stringify(patch.assigneeIds) && (current.event.location ?? '') === (patch.location ?? '') && (current.event.notes ?? '') === (patch.notes ?? '')
    const revision = unchanged ? currentRevision : currentRevision + 1
    if (!unchanged) await ctx.db.patch(current.event._id, { ...patch, revision })
...
export const setPreferences = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.peopleFilter) {
      ...
      if (args.peopleFilter.userIds.some((id) => !allowed.has(id))) throw new Error('calendar:person-not-member')
    }
    const patch: { calendarView?: 'month' | 'week' | 'agenda'; calendarPeopleFilter?: { userIds: Id<'users'>[]; includeUnassigned: boolean } } = {}
    if (args.view !== undefined) patch.calendarView = args.view
    if (args.peopleFilter !== undefined) patch.calendarPeopleFilter = args.peopleFilter ?? undefined
