...
function EditBaby() {
  ...
  return <EditBabyForm key={baby._id} baby={baby} spaceSlug={spaceSlug} />
}
        ...
        <button
          ...
          onClick={async () => {
            if (!window.confirm(`Delete ${baby.name} and all logged entries?`))
              return
            await remove({ spaceSlug, id: baby._id })
            navigate({ to: '/s/$spaceSlug/baby-log', params: { spaceSlug } })
          }}
        >
          Delete
        </button>
      ...
      <BabyForm
        ...
        onSubmit={async (values) => {
          ...
          try {
            ...
            navigate({
              to: '/s/$spaceSlug/baby-log/$babyId',
              params: { spaceSlug, babyId: baby._id },
            })
          } catch (err) {
            setError(
