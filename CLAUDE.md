...
- **Convex** backend (`convex/`) — functions: `recipes.ts`, `groups.ts`, `users.ts`,
  `recipeImport.ts`, `recipeNutrition.ts`, `taskLists.ts`, `tasks.ts`, `integrations.ts`,
  `lib/sharing.ts`, `lib/nutrition.ts`, `lib/taskAccess.ts`, `lib/taskProviders/`
  (adapter pattern for Notion/Todoist). Schema in `convex/schema.ts`.
